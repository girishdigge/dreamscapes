// services/express/utils/logger.js
const fs = require('fs');
...
class Logger {
  ...
  formatConsoleMessage(logObj) {
    const { timestamp, level, message, meta } = logObj;
  ...
  // Dream processing logging
  logDreamProcessing(dreamId, action, duration, success = true, meta = {}) {
    const logData = {
      dreamId,
      action,
    ...
    if (success) {
      this.info(`Dream ${action}`, logData);
    } else {
      this.error(`Dream ${action} failed`, logData);
    }
  }
...
function startResourceMonitoring(interval = 60000) {
  // 1 minute
  setInterval(() => {
...
// Dream-specific logging utilities
const dreamLogger = {
  generated(dreamId, style, duration, source) {
    logger.info('Dream Generated', {
      dreamId,
      style,
  ...
  patched(dreamId, editText, duration, changes) {
    logger.info('Dream Patched', {
      dreamId,
      editText,
      processingTime: duration + 'ms',
  ...
  exported(dreamId, format, duration, fileSize) {
    logger.info('Dream Exported', {
      dreamId,
      format,
      processingTime: duration + 'ms',
  ...
  cached(operation, cacheKey, hit = true) {
